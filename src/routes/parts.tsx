...
// ---- Wheels/rims catalog (fallback: raw-file editor) ----
...
// explicitly don't cover it ("wheels/tires (wheels-500.xml) aren't covered", parts.mjs:181).
// scripts/seed-cms-catalog.mjs seeds it under category "parts", seedKey "wheels-500.xml". There's
// no per-field wheel editor route on the backend, so this falls back to the existing raw-content
// CMS file editor (GET/PUT /api/admin/cms/files/:id, cmsApi) already used elsewhere in this app.
function WheelsEditor() {
  ...
  const save = async () => {
    ...
    try {
      ...
      toast.success("Saved wheels-500.xml.");
    } catch (err) {
      toast.error(err instanceof ApiError ? err.reason || err.message : "Save failed.");
    } finally {
      setSaving(false);
  ...
  return (
    <Panel
      ...
      meta="RAW FILE — wheels-500.xml (no per-field editor on the backend)"
      delay={300}
      className="mt-8"
    >
      <div className="flex items-center justify-between px-6 py-4">
        <div className="font-mono text-[11px] text-mute">
          Wheels/tires pricing and designs live in a separate catalog file from parts-full.xml, with
          no structured price editor route. Edit the raw XML directly below.
        </div>
        <button
      ...
      {expanded ? (
        <div className="border-t border-line bg-raise/10 px-6 py-5">
          {list.isLoading || (meta && file.isLoading) ? (
            <div className="font-mono text-[11px] text-dim">Loading wheels-500.xml...</div>
            ...
            <>
              <textarea
                ...
                spellCheck={false}
                className="h-80 w-full resize-y rounded border border-line bg-background p-4 font-mono text-[11px] leading-relaxed text-foreground outline-none focus:border-accent/50"
              />
              <div className="mt-3 flex justify-end">
                <button
                  ...
                  className="h-9 rounded bg-accent px-5 text-[11px] font-bold tracking-widest text-accent-foreground uppercase transition-all hover:brightness-125 disabled:opacity-50"
                >
                  {saving ? "Saving..." : "Save wheels-500.xml"}
                </button>
              </div>
            </>
          )}
